...
Run:
```bash
grep -rniE "type: (source|entity|concept|query|discovery|story|index|log|brainstorm)\b" skills/ || echo "no lowercase type values"
grep -rn "context_pages" skills/ || echo "no context_pages"
grep -rnE "^raw: |^url: " skills/ || echo "no raw:/url: keys"
```
Expected: each prints its "clean" message, except where `arche-lint/references/OKF-CONFORMANCE.md` names them as *detection targets*, which is correct and expected.
