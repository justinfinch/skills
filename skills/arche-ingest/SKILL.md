...
1. **Land the raw file in `./.arche/raw/`.** Derive the slug per SCHEMA's "Slug derivation" rules. Whichever path applies, the invariant is the same: when this step finishes, nothing under `raw/` ends in `.md`.
   - **Already in `raw/`** (batch mode) → nothing to copy. If the filename ends in `.md`, rename it to `.txt` before going further, and use the new name everywhere downstream — including the `resource:` written in step 3. Skipping this is what leaves a bundle non-conformant the moment ingest finishes: every `.md` file must carry frontmatter (§11 rule 1), `raw/` files are immutable, so the only repair left is `/arche-lint`'s H1 rename plus a rewrite of the `resource:` this run just wrote. Normalizing here costs nothing.
...
3. **Write the source summary** at `.arche/sources/<slug>.md` using this skill's [source.template.md](assets/source.template.md) as the layout.
   - Set `resource:` to the canonical URL for a web source, or to `../raw/<slug>.<ext>` — a path relative to the page containing it, where `<ext>` is the extension actually **present in `raw/` after step 1**, never the incoming one: it is `.txt` whenever the source was a URL or pasted text, whenever the incoming file was `.md`, and whenever step 1 renamed a `.md` file that was already sitting in `raw/` — for a file-only source. When both exist, `resource:` is the URL and the snapshot becomes a `sources` entry with `id: snapshot`.
