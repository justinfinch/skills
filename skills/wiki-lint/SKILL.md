...
3. **Orphan pages.** Any entity/concept/query/discovery page with zero inbound links from other wiki pages (excluding `index.md`) → flag. New pages get a grace pass — only flag if `created:` is older than 14 days. Discoveries are expected to be linked from at least the concept/entity pages that promoted their top ideas; a discovery with no inbound links after the grace period likely means no promotions stuck.
...
7. **Index/log integrity.** Every page in `./.wiki/{sources,entities,concepts,queries,discoveries}/` should appear in `index.md`. Every `ingest`/`query`/`discovery` entry in `log.md` should reference pages that exist. Flag mismatches both directions.
