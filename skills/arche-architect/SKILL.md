...
7. **Insert into `log.md`** with op `architect`. Insert a `- **Architect**: …` bullet immediately above the topmost `## YYYY-MM-DD` heading, creating today's heading if it is absent. The bullet's prose names the system, a one-line summary of what was decided, the ADR count, and every page touched.
