...
3. Write a per-directory `index.md` into each of `sources/`, `entities/`, `concepts/`, `queries/`, `discoveries/`, and `stories/` from `assets/subindex.template.md`, replacing `{{TITLE}}` with that directory's singular type name (`Source`, `Entity`, `Concept`, `Query`, `Discovery`, `Story`). These carry **no frontmatter** — §8 permits it only on the bundle-root index.
   Because of that, the root index's own entries — the six pointing at these subdirectory indexes, plus the `# Bundle` entries for `SCHEMA.md` and `log.md` — have no `description` behind them. Their glosses are hand-written navigation text, written once here and stable thereafter. `/arche-lint`'s S2 gloss check is scoped to skip them; leave them as the template has them rather than restating a page description.
